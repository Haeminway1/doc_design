...
- `text-shadow` - **텍스트 뒤에 어두운 선택 박스 생성** (가장 흔한 문제)
- `backdrop-filter` - **PDF 전체가 백지로 출력됨** (치명적! blur() 사용 시 전 페이지 렌더링 실패)
- `-webkit-backdrop-filter` - 위와 동일
- `rgba()` 투명도 - 예측 불가능한 결과
- `opacity` - 색상이 이상하게 보일 수 있음

### ⚠️ 치명적 CSS 속성 (2026-02-25 추가)

| 속성 | 증상 | 심각도 |
|------|------|--------|
| `backdrop-filter: blur()` | PDF 전체 백지 출력 (파일 크기 ~185KB로 축소) | **치명적** — 전체 문서 렌더링 실패 |
| `text-shadow` + `rgba()` | 텍스트 뒤에 어두운 박스 (드래그 선택한 것처럼 보임) | **높음** — 시각적 결함 |
| `background-blend-mode` | 배경 렌더링 실패 | **높음** |

**진단법**: PDF 파일이 비정상적으로 작으면 (수백 페이지인데 200KB 이하) `backdrop-filter`를 의심하세요.
